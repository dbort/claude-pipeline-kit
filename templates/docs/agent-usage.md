...
3. Review the generated `tasks/active/{TASK_PREFIX}-XXX-[slug].md`. Check
   in particular:
...
4. Once you're satisfied, tell the Planner to advance the task (it will set
   ...
   `/dispatch-tasks {TASK_PREFIX}-XXX` against it directly — naming a
...
task — `/dispatch-tasks {TASK_PREFIX}-XXX`. Once you name a task, it runs
...
- **One-shot:** ask Claude to "invoke the dispatch-tasks skill on
  {TASK_PREFIX}-XXX" to nudge that task forward once.
- **Autonomous:** run `/loop invoke the dispatch-tasks skill
  {TASK_PREFIX}-XXX` and let it self-pace against that same task. It stops
...
1. Check out the task's `{TASK_PREFIX}-XXX` branch and exercise the task
   yourself; passing tests and lint don't guarantee it does what you
   wanted.
2. If it's good: run `/approve-task {TASK_PREFIX}-XXX`. Running it against
   a task IS the sign-off: it finalizes the task file, re-sweeps the
   branch with `doc-hygiene`, and merges `{TASK_PREFIX}-XXX` into `main`
...
`tasks/active/{TASK_PREFIX}-XXX-REVIEW.md` for what it kept rejecting.
Your options:
- Fix the underlying issue yourself on the `{TASK_PREFIX}-XXX` branch,
