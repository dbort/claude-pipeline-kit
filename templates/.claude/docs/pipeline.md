...
This file is the single authoritative definition of the {TASK_PREFIX}-XXX
...
```sh
# PIPELINE-KIT-SETUP-TODO(verification-fast): replace the line below with this repo's real fast checks (e.g. `npm run lint && npm test`, `cargo clippy && cargo test`), then delete this marker line.
./validate.sh
```
...
```sh
# PIPELINE-KIT-SETUP-TODO(verification-full): list this repo's slow/live-infra checks here (e.g. `./e2e.sh`), or leave the list empty if the repo has no full tier yet; either way, delete this marker line to record the decision.
```
...
| `implementation` | Implementer | Executes the task file's `## Execution Plan` steps in order on the task's `{TASK_PREFIX}-XXX` branch, then hands off to `review`. |
...
| `user_signoff` | Human | The user tests the branch manually, then runs `/approve-task {TASK_PREFIX}-XXX` — invoking that skill against a task IS the sign-off act. It finalizes the task file, re-sweeps with `doc-hygiene`, and merges into `main` only after the merged result passes both verification tiers. |
...
`tasks/active/{TASK_PREFIX}-XXX-REVIEW.md` (same id as the task file, so
...
mid-interview, or via `dispatch-tasks {TASK_PREFIX}-XXX` naming a task
...
- **Acts only on a named task.** `dispatch-tasks {TASK_PREFIX}-XXX` takes
...
- **Loop mode:** driven by `/loop invoke the dispatch-tasks skill
  {TASK_PREFIX}-XXX`, it self-paces, re-supplying the same task id on each
  wake-up, and ends itself once that task leaves `tasks/active/`.

## Git branching

- Each task's work happens on a branch named exactly `{TASK_PREFIX}-XXX`
...
- **Never commit task work directly to `main`.** Work lands on `main` only
  via `approve-task` merging a `{TASK_PREFIX}-XXX` branch, and that merge
  ...
  history.
- A task's phase-transition commits live on its `{TASK_PREFIX}-XXX`
  ...
  state via `git show {TASK_PREFIX}-XXX:tasks/active/{TASK_PREFIX}-XXX-*.md`
  (details in `dispatch-tasks`'s Step 1).

## Task dependencies (`blocked_by`)

A task file may declare `blocked_by: [{TASK_PREFIX}-XXX, ...]` in its
...
- **`tasks/active/`** — open tasks, one `{TASK_PREFIX}-XXX-[slug].md`
