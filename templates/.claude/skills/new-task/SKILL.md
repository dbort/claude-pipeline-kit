...
### Step 0: Setup gate
If `pipeline-setup.sh` exists at the repo root, run it via Bash. If it exits nonzero, stop: report the pending setup steps it printed and tell the user to finish repo setup before planning tasks — a task planned against placeholder verification commands or empty conventions produces garbage downstream. If the script is absent, setup is complete; proceed.
...
2. **Strict Step Isolation:** Break the code generation into separate files. Ensure step $N$ does not depend on uncompleted files in step $N+1$.
...
File name: `tasks/active/{TASK_PREFIX}-XXX-[human-readable-slug].md`
...
review_rejections: 0
# blocked_by: [{TASK_PREFIX}-YYY]   # only for a genuine hard blocker — see rule 5 above; omit this line otherwise
---

# {TASK_PREFIX}-XXX: [Short Task Title]
